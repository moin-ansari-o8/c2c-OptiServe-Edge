...
with torch.no_grad():
    ...
    for step in range(max_new_tokens):
        ...
        # Extract the final transformer layer hidden state
        final_hidden_state = outputs.hidden_states[-1][:, -1, :]

        # Calculate activation features
        activation_mean = final_hidden_state.mean().item()

        activation_variance = final_hidden_state.var(
            unbiased=False
        ).item()

        activation_l2_norm = torch.linalg.vector_norm(
            final_hidden_state,
            ord=2,
            dim=-1
        ).item()
        ...
        print(
            ...
            f"Mean: {activation_mean:.6f} | "
            f"Variance: {activation_variance:.6f} | "
            f"L2 Norm: {activation_l2_norm:.6f}"
        )
